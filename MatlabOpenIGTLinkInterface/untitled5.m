...
S_ind = 4;                  %-- Starting index
E_ind = 4;                  %-- Ending index
%--------------------------------------------------------------------------
...
%     load(Phantom_name)
%     [phantom_positions, phantom_amplitudes] = cyst_move(round((rand*50) + 10));
%     phantom_amplitudes(phantom_positions(:,3)<(20/1000)) =  phantom_amplitudes(phantom_positions(:,3)<(20/1000)) / 5;
    ...
    angle_width = 14;  %-- in degrees 
    N_angles = 75;          %-- Number of Angels
...
%     z_cp(Index) = [];
    ...
    t = toc
    envelope_beamformed_data = abs(reshape(beamformed_data,[numel(scan.z_axis) 128]));
...
%     log_env(log_env<-setdB) = -setdB;
%     imagesc(log_env(1:1300,:)), colormap gray
log_com_plot(envelope_beamformed_data,1,1300);
    title('Target')
